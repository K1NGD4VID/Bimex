# Documentación Bimex

Índice completo de la documentación del proyecto.

## 📚 Documentación general

| Documento | Descripción |
|-----------|-------------|
| [FAQ (Español)](faq-es.md) | Preguntas frecuentes en español |
| [FAQ (English)](faq-en.md) | Frequently asked questions |
| [Guía del contribuidor](guia-contribuidor.md) | Cómo apoyar un proyecto como usuario |
| [Guía del creador de proyecto](guia-creador-proyecto.md) | Cómo lanzar un proyecto en Bimex |
| [Guía técnica de integración](guia-tecnica-integracion.md) | ABI, CLI, SDK para desarrolladores |

## 🔐 Seguridad y bug bounty

| Documento | Descripción |
|-----------|-------------|
| [Security Policy / Bug Bounty](../SECURITY.md) | Política de seguridad, programa de recompensas Tier 1 y proceso de divulgación responsable |
| [Threat Model](THREAT-MODEL.md) | Modelo de amenazas, activos, actores y superficie de ataque |
| [Contract Invariants](CONTRACT-INVARIANTS.md) | Invariantes críticos del contrato inteligente |
| [Bug Bounty Tier 2](SECURITY-BOUNTY-TIER2.md) | Decisión de plataforma y presupuesto para el programa post-lanzamiento |

## 🚀 Proyecto Piloto (Mainnet)

| Documento | Descripción | Audiencia |
|-----------|-------------|-----------|
| [**RESUMEN EJECUTIVO**](RESUMEN-PROYECTO-PILOTO.md) | **Empieza aquí** - Overview completo | Todos |
| [**Flujo visual**](FLUJO-VISUAL-PILOTO.md) | Diagrama paso a paso del proceso | Todos |
| [**FAQ del piloto**](faq-proyecto-piloto.md) | Preguntas frecuentes específicas | Todos |
| [Guía del proyecto piloto](guia-proyecto-piloto.md) | Proceso completo de onboarding | Equipo Bimex |
| [Checklist ejecutivo](checklist-ejecutivo-piloto.md) | Lista de verificación paso a paso | Project Manager |
| [Tracking del piloto](proyecto-piloto-tracking.md) | Documento vivo para seguimiento | Equipo Bimex |
| [Templates de comunicación](templates-comunicacion-piloto.md) | Emails, posts, anuncios listos | Marketing |
| [Plan de marketing](plan-marketing-piloto.md) | Estrategia de comunicación completa | Marketing |

## 🛠️ Scripts y herramientas

Ver [scripts/README.md](../scripts/README.md) para documentación de scripts de ayuda.

## 📖 Documentación técnica completa

Ver [DOCUMENTACION.txt](../DOCUMENTACION.txt) en la raíz del proyecto para:
- Arquitectura del sistema
- Smart contract (tipos, funciones, storage)
- Ciclo de vida de proyectos
- Cálculo de yield
- Despliegue en Testnet y Mainnet
- Variables de entorno
- Changelog completo

## 🎯 Guías rápidas

### Para contribuyentes
1. Lee el [FAQ](faq-es.md)
2. Sigue la [Guía del contribuidor](guia-contribuidor.md)
3. Conecta tu wallet y contribuye

### Para creadores de proyectos
1. Lee el [FAQ](faq-es.md)
2. Sigue la [Guía del creador de proyecto](guia-creador-proyecto.md)
3. Prepara tus documentos y lanza tu proyecto

### Para desarrolladores
1. Lee la [Guía técnica de integración](guia-tecnica-integracion.md)
2. Revisa [DOCUMENTACION.txt](../DOCUMENTACION.txt)
3. Explora el código en [bimex/contracts/bimex/src/](../bimex/contracts/bimex/src/)

### Para el equipo Bimex (proyecto piloto)
1. Lee el [RESUMEN EJECUTIVO](RESUMEN-PROYECTO-PILOTO.md)
2. Abre el [Checklist ejecutivo](checklist-ejecutivo-piloto.md)
3. Sigue el proceso paso a paso

## 🔗 Enlaces útiles

- **Repositorio**: https://github.com/David1984TK/Bimex
- **Frontend (Testnet)**: https://bimex-frontend.vercel.app
- **Stellar Docs**: https://developers.stellar.org
- **Soroban Docs**: https://soroban.stellar.org/docs
- **Freighter Wallet**: https://www.freighter.app

## 📝 Contribuir a la documentación

Si encuentras errores o quieres mejorar la documentación:

1. Abre un issue en GitHub
2. Propón cambios vía pull request
3. Contacta al equipo directamente

## 📅 Última actualización

**Fecha**: 2026-04-28  
**Versión**: 2.0  
**Estado**: Activo (Testnet) | Preparando Mainnet

---

**¿Tienes dudas?** Revisa el [FAQ](faq-es.md) o contacta al equipo.
